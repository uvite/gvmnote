...
func (menuService *MenuService) getMenuTreeMap(role_id uint) (treeMap map[int64][]system.SysMenu, err error) {
	var allMenus []system.SysMenu
	var baseMenu []system.SysBaseMenu
	var btns []system.SysAuthorityBtn
	treeMap = make(map[int64][]system.SysMenu)
	...
	err = global.GVA_DB.Where("id in (?)", MenuIds).Order("sort desc").Preload("Parameters").Find(&baseMenu).Error
	if err != nil {
		return
	}
	...
	for i := range baseMenu {
		...
		allMenus = append(allMenus, system.SysMenu{
			...
			AuthorityId: role_id,
			MenuId:      (int64(baseMenu[i].ID)),
			Parameters:  baseMenu[i].Parameters,
		})
	}

	err = global.GVA_DB.Where("authority_id = ?", role_id).Preload("SysBaseMenuBtn").Find(&btns).Error
	if err != nil {
		return
	}
	...
	for _, v := range btns {
		...
		btnMap[v.SysMenuID][v.SysBaseMenuBtn.Name] = role_id
	}
...
func (menuService *MenuService) getChildrenList(menu *system.SysMenu, treeMap map[int64][]system.SysMenu) (err error) {
...
func (menuService *MenuService) getBaseChildrenList(menu *system.SysBaseMenu, treeMap map[int64][]system.SysBaseMenu) (err error) {
	menu.Children = treeMap[int64(menu.ID)]
	for i := 0; i < len(menu.Children); i++ {
...
func (menuService *MenuService) getBaseMenuTreeMap() (treeMap map[int64][]system.SysBaseMenu, err error) {
	var allMenus []system.SysBaseMenu
	treeMap = make(map[int64][]system.SysBaseMenu)
